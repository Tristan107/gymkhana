...
- `npm test`              # vitest, EXCLUDES ai_mcts.test.ts (slow/AI)
...
## Structure
- `src/game/`      pure game logic, no React (logic.ts, reducer.ts, ai_mcts.ts)
...
- AI behavior is rule-based with MCTS. The spec lives in `.opencode/01_simplified_rules_for_ai.md` and
  `.opencode/02_basic_heuristics.md` — read these before changing `ai_mcts.ts`.
- Board: 11x11, orthogonal adjacency only, fixed staggered start tokens, 20 tokens/player.
- Do NOT add `ai_mcts.test.ts` to the default `npm test` run
